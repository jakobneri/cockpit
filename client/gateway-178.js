...
const GATEWAY_IP = '192.168.178.1';
const GATEWAY_USER = 'admin';
const GATEWAY_PASS = 'your_password_here'; // Replace with your actual password
const DB_URL = process.env.DB_URL || 'http://localhost:3001';
const HOSTNAME = `${GATEWAY_IP}-dedicated-gateway-client`;
...
async function report() {
  try {
    ...
    if (response.ok) {
      log.report(`Reporting Successful for ${GATEWAY_IP}`);
    }
  } catch (err) {
